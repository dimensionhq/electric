...
def unzip_file(download_dir: str, unzip_dir_name: str = ''):
    ...
    return unzip_dir_name.replace('.zip', '')
...
def create_start_menu_shortcut(file_name, shortcut_name):
    start_menu = os.environ['APPDATA'] + R'\Microsoft\Windows\Start Menu\Programs\Electric'
    if not os.path.isdir(start_menu):
        os.mkdir(start_menu)
    path = os.path.join(start_menu, f'{shortcut_name}.lnk')
    os.chdir(loc + unzip_dir)
    icon = loc + unzip_dir + '\\' + file_name
    shell = win32com.client.Dispatch("WScript.Shell")
    shortcut = shell.CreateShortCut(path)
    shortcut.Targetpath = loc + unzip_dir + '\\' + file_name
    shortcut.IconLocation = icon
    shortcut.WindowStyle = 7 # 7 - Minimized, 3 - Maximized, 1 - Normal
    shortcut.save()

def generate_shim(unzip_dir: str, exe_path: str):
    if not os.path.isdir('shims'):
        os.mkdir('shims')
    os.chdir(unzip_dir)
    loc = __file__.replace('zip-install.py', '') + r'\shims\\'
    file_name = exe_path.replace('.exe', '').replace('.bat', '').replace('.cmd', '').replace('.ps1', '')
    with open(rf'{loc}{file_name}.bat', 'w+') as f:
        f.write(f'@echo off\n{exe_path}')