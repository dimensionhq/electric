...
def cli(_):
    ...
    if not os.path.isfile(rf"{PathManager.get_appdata_directory()}\settings.json"):
        click.echo(
            click.style(
                f"Creating settings.json at {Fore.LIGHTCYAN_EX}{PathManager.get_appdata_directory()}{Fore.RESET}",
                fg="bright_green",
            )
        )
...
@click.option(
    "--version", "-v", type=str, help="Install a certain version of a package"
)
@click.option(
    "--nightly",
    "--pre-release",
    is_flag=True,
    help="Install a nightly or pre-release build of a package",
)
@click.option(
    "--portable",
    "--non-admin",
    "-p",
    is_flag=True,
    help="Install a portable version of a package",
)
@click.option(
    "--verbose", "-vb", is_flag=True, help="Enable verbose mode for installation"
)
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Disable progress bar for installation",
)
@click.option(
    "--update",
    "-up",
    is_flag=True,
    default=False,
    help="Update electric before installation",
)
@click.option(
    "--no-color", "-nc", is_flag=True, help="Disable colored output for installation"
)
...
@click.option(
    ...
    "-dir",
    "install_directory",
    help="Specify an installation directory for a package",
)
@click.option(
    "--virus-check", "-vc", is_flag=True, help="Check for virus before installation"
)
@click.option(
    "-y", "--yes", is_flag=True, help="Accept all prompts during installation"
)
@click.option(
    "--silent",
    "-s",
    is_flag=True,
    help="Completely silent installation without any output to console",
)
@click.option(
    "--vscode",
    "-vs",
    is_flag=True,
    help="Specify a Visual Studio Code extension to install",
)
@click.option(
    "--sublime",
    "-sb",
    is_flag=True,
    help="Specify a Sublime Text 3 extension to install",
)
@click.option(
    "--atom", "-ato", is_flag=True, help="Specify an Atom extension to install"
)
@click.option(
    "--python", "-py", is_flag=True, help="Specify a Python package to install"
)
...
@click.option(
    ...
    "-sc",
    is_flag=True,
    help="Force downloads and installations one after another",
)
@click.option(
    "--reduce",
    "-rd",
    is_flag=True,
    help="Cleanup all traces of package after installation",
)
...
@click.option(
    ...
    "-f",
    is_flag=True,
    help="Force install a package, ignoring any existing installations of a package.",
)
@click.option(
    "--configuration", "-cf", is_flag=True, help="Specify a config file to install"
)
...
@click.option(
    ...
    "-m",
    "manifest",
    help="Read from a manifest file instead of querying from the community repository",
)
@click.pass_context
def install(
    ctx,
    ...
    if len(packages) > 1 and manifest:
        write(
            "Cannot Install Multiple Packages From A Single Manifest. Make sure you install only 1 package at a time while specifying --manifest",
            "bright_red",
            metadata,
        )
        sys.exit()
    ...
    if not manifest:
        ...
        corrected_package_names = list(
            set(get_autocorrections(packages, get_correct_package_names(), metadata))
        )
    else:
    ...
    for package in corrected_package_names:
        ...
        if not manifest:
            ...
            try:
                ...
                write_debug(
                    f"{manifest} FileNotFoundError, Specified Manifest Cannot Be Found!",
                    metadata,
                )
                log_info(
                    f"{manifest} FileNotFoundError, Specified Manifest Cannot Be Found!",
        ...
        if not metadata.silent:
            if not metadata.no_color:
                print(
                    "SuperCached [",
                    Fore.LIGHTCYAN_EX + f'{res["display-name"]}' + Fore.RESET + " ]",
                )
            else:
        ...
        packet = Packet(
            ...
            pkg["default-install-dir"].replace(
                "<appdata>", os.environ["APPDATA"].replace("\\Roaming", "")
            )
            if "default-install-dir" in list(pkg.keys())
            else None,
        ...
        log_info(
            f"Rapidquery Successfully Received {packet.json_name}.json",
            metadata.logfile,
        )
        ...
        write_verbose(f"Finding closest match to {packet.json_name}...", metadata)
        ...
        log_info(
            f"Installing {packet.display_name} through Setup{packet.win64_type}",
            metadata.logfile,
        )
            ...
            if isinstance(pkg["pre-install"], list):
                ...
                for proc in pkg["pre-install"]:
                    ...
                    if proc["type"] == "powershell":
                        with open(
                            rf"{tempfile.gettempdir()}\electric\temp.ps1", "w+"
                        ) as f:
                            for line in proc["code"]:
                                line = (
                                    line.replace("<installer>", configs["path"])
                                    .replace("<package-name>", packet.json_name)
                                    .replace("<display-name>", packet.display_name)
                                    .replace("<version>", version)
                                    .replace(
                                        "<directory>",
                                        packet.directory
                                        if packet.directory != None
                                        else "",
                                    )
                                    .replace("<temp>", tempfile.gettempdir())
                                )
                                line += "\n"
                                f.write(line)
                    ...
                    if proc["type"] == "cmd":
                        with open(
                            rf"{tempfile.gettempdir()}\electric\temp.bat", "w+"
                        ) as f:
                            for line in proc["code"]:
                                line = (
                                    line.replace("<installer>", configs["path"])
                                    ...
                                    .replace("<display-name>", packet.display_name)
                                    .replace("<version>", version)
                                    .replace("<directory>", packet.directory)
                                    .replace("<temp>", tempfile.gettempdir())
                                )
                                line += "\n"
                                f.write(line)
                    ...
                    if proc["type"] == "python":
                        ...
                        for line in proc["code"]:
                            add = (
                                ...
                                .replace(
                                    "<directory>",
                                    packet.directory if packet.directory else "",
                                )
                                .replace("<version>", version)
                                + "\n"
                            )
                            ...
                            if f"{packet.win64_type}{packet.win64_type}" in add:
                                add = add.replace(
                                    f"{packet.win64_type}{packet.win64_type}",
                                    f"{packet.win64_type}",
                                )
                    ...
                    if "override" in list(proc.keys()):
                        if (
                            proc["override"] == True
                            and len(corrected_package_names) == 1
                        ):
                            sys.exit()
                        elif (
                            len(corrected_package_names) > 1
                            and proc["override"] == True
                        ):
                            continue
        ...
        if (
            not "pre-install" in list(pkg.keys())
            or packet.win64_type not in configs["path"]
        ):
            setup_name = configs["path"].split("\\")[-1] + packet.win64_type

        status = "Installing"

        add_hotkey("ctrl+c", lambda: handle_exit(status, setup_name, metadata))
        ...
        if virus_check or metadata.settings.virus_check:
            ...
            if not metadata.silent:
                with Halo(
                    text="Scanning File For Viruses ",
                    text_color="cyan" if not metadata.no_color else "white",
                    color="green" if not metadata.no_color else "white",
                ) as h:
        ...
        write(f"Installing {packet.display_name}", "bright_cyan", metadata)
        log_info(
        ...
        if not get_pid(setup_name):
            ...
            write_verbose(
                f'Running {packet.display_name} Installer at {configs["path"]}',
                metadata,
            )
            log_info(
                f'Running {packet.display_name} Installer at {configs["path"]}',
        ...
        if packet.shim:
            ...
            for shim in packet.shim:
                ...
                shim = shim.replace("<install-directory>", replace_install_dir).replace(
            ...
            generate_shim(
                shim,
                shim_name.replace("<version>", packet.version),
                shim.split(".")[-1],
            )
        ...
        if packet.add_path:
            ...
            write(
                f'Appending "{packet.add_path.replace("<install-directory>", replace_install_dir)}" To PATH',
                "bright_green",
                metadata,
            )
            write_verbose(
            ...
            append_to_path(
                packet.add_path.replace("<install-directory>", replace_install_dir)
            )
        ...
        if packet.set_env:
            if isinstance(packet.set_env, list):
                for obj in packet.set_env:
                    ...
                    write_verbose(
                        f'Setting Environment Variable {name} to {obj["value"].replace("<install-directory>", replace_install_dir)}',
                        metadata,
                    )
                    log_info(
                        f'Setting Environment Variable {name} to {obj["value"].replace("<install-directory>", replace_install_dir)}',
                ...
                write_verbose(
                    f'Setting Environment Variable {name} to {packet.set_env["value"].replace("<install-directory>", replace_install_dir)}',
                    metadata,
                )
                log_info(
                    f'Setting Environment Variable {name} to {packet.set_env["value"].replace("<install-directory>", replace_install_dir)}',
                ...
                set_environment_variable(
                    name,
                    packet.set_env["value"].replace(
                        "<install-directory>", replace_install_dir
                    ),
                )

        if (
            final_snap.env_length > start_snap.env_length
            or final_snap.sys_length > start_snap.sys_length
            or changes_environment
        ):

            write(
                "The PATH environment variable has changed. Run `refreshenv` to refresh your environment variables.",
                "bright_green",
                metadata,
            )
        ...
        if metadata.reduce_package:
            ...
            write_debug(
                f'Deleting installer files at {tempfile.gettempdir()}. Path : ({configs["path"]}{packet.win64_type})',
                metadata,
            )
            try:
            ...
            write(
                "Successfully Cleaned Up Installer From Temp Directory",
                "bright_green",
                metadata,
            )
        ...
        if not packet.run_test:
            ...
            log_info(
                f"No Pre-Defined Tests found for {packet.display_name}",
                metadata.logfile,
            )

            write(f"Running Tests For {packet.display_name}", "bright_white", metadata)

            write_debug(
                f"All Pre-Defined checks for {packet.display_name} passed. Registering successful package installation",
                metadata,
            )
            write_verbose(
            ...
            log_info(
                f"All Pre-Defined checks for {packet.display_name} passed",
                metadata.logfile,
            )
            if not metadata.no_color:
                write(
                    f"[{Fore.LIGHTGREEN_EX} OK {Fore.RESET}] Pre-Defined Checks",
                    "bright_white",
                    metadata,
                )
            else:
            ...
            write(
                f"Successfully Installed {packet.display_name}",
                "bright_magenta",
                metadata,
            )
            ...
            log_info(
                f"Running pre-defined checks found for {packet.display_name}",
                metadata.logfile,
            )
            write(f"Running Tests For {packet.display_name}", "bright_white", metadata)
            if find_existing_installation(packet.json_name, packet.display_name):
                if not metadata.no_color:
                    write(
                        f"[ {Fore.LIGHTGREEN_EX}OK{Fore.RESET} ]  Registry Check",
                        "bright_white",
                        metadata,
                    )
                else:
                ...
                if find_existing_installation(packet.json_name, packet.display_name):
                    ...
                    write(
                        f"Failed To Install {packet.display_name}",
                        "bright_red",
                        metadata,
                    )
                sys.exit()
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Disable progress bar for update",
)
@click.option(
    "--no-color", "-nc", is_flag=True, help="Disable colored output for update"
)
...
@click.option(
    ...
    "-s",
    is_flag=True,
    help="Completely silent update without any output to console",
)
@click.option(
    "--virus-check", "-vc", is_flag=True, help="Check for virus before bundle update"
)
@click.option("--rate-limit", "-rl", type=int, default=-1)
...
@click.option(
    ...
    "-rd",
    is_flag=True,
    help="Cleanup all traces of package after bundle update",
)
    ...
    for package in corrected_package_names:
        ...
        if portable:
            ...
            data = {
                ...
                "shortcuts": pkg[pkg["latest-version"]]["shortcuts"]
                if "shortcuts" in keys
                else [],
                "pre-install": pkg[pkg["latest-version"]]["pre-install"]
                if "pre-install" in keys
                else [],
                "post-install": pkg[pkg["latest-version"]]["post-install"]
                if "post-install" in keys
                else [],
                "install-notes": pkg[pkg["latest-version"]]["install-notes"]
                if "install-notes" in keys
                else None,
                "uninstall-notes": pkg[pkg["latest-version"]]["uninstall-notes"]
                if "uninstall-notes" in keys
                else None,
                "set-env": pkg[pkg["latest-version"]]["set-env"]
                if "set-env" in keys
                else None,
                "persist": pkg[pkg["latest-version"]]["persist"]
                if "persist" in keys
                else None,
                "checksum": pkg[pkg["latest-version"]]["checksum"]
                if "checksum" in keys
                else None,
                "dependencies": pkg[pkg["latest-version"]]["dependencies"]
            ...
            if check_newer_version(package, packet, installed_packages_dict):
                ...
                if not yes:
                    if not local:
                        ...
                        write(
                            rf"There is a newer version of {packet.display_name} Availiable ({installed_version}) => ({packet.version})",
                            "bright_yellow",
                            metadata,
                        )
                        sys.exit()
                    ...
                    if isinstance(packet.pre_update, list):
                        ...
                        for proc in packet.pre_update:
                            ...
                            if proc["type"] == "python":
                                ...
                                for line in proc["code"]:
                                    add = (
                                        line.replace("<temp>", tempfile.gettempdir())
                                        .replace("<package-name>", packet.json_name)
                                        .replace("<display-name>", packet.display_name)
                                        .replace(
                                            "<directory>",
                ...
                if continue_update:
                    ...
                    write(
                        f"Successfully Updated {package} to latest version.",
                        "bright_green",
                        metadata,
                    )
...
@click.option(
    "--verbose", "-v", is_flag=True, help="Enable verbose mode for uninstallation"
)
@click.option(
    "--debug", "-d", is_flag=True, help="Enable debug mode for uninstallation"
)
@click.option(
    "--no-color", "-nc", is_flag=True, help="Disable colored output for uninstallation"
)
@click.option("--log-output", "-l", "logfile", help="Log output to the specified file")
@click.option(
    "-y", "--yes", is_flag=True, help="Accept all prompts during uninstallation"
)
@click.option(
    "--silent",
    "-s",
    is_flag=True,
    help="Completely silent uninstallation without any output to console",
)
@click.option(
    "--python", "-py", is_flag=True, help="Specify a Python package to uninstall"
)
@click.option(
    "--atom", "-ato", is_flag=True, help="Specify an Atom extension to uninstall"
)
@click.option(
    "--vscode",
    "-vs",
    is_flag=True,
    help="Specify a Visual Studio Code extension to uninstall",
)
@click.option(
    "--nightly",
    "--pre-release",
...
@click.option(
    "--node", "-npm", is_flag=True, help="Specify a Python package to uninstall"
)
@click.option(
    "--portable",
    "--non-admin",
    "-p",
    is_flag=True,
    help="Uninstall a portable version of a package",
)
@click.option("--plugin", "-pg", is_flag=True, help="Uninstall a plugin of a package")
@click.option(
    "--configuration", "-cf", is_flag=True, help="Specify a config file to uninstall"
)
...
def uninstall(
    ...
    for package in corrected_package_names:
        installed_packages = [
            "".join(f.replace(".json", "").split("@")[:1])
            for f in os.listdir(PathManager.get_appdata_directory() + r"\Current")
        ]
        portable_installed_packages = [
            "".join(f.split("@")[:1])
            for f in os.listdir(os.path.expanduser("~") + r"\electric")
        ]
            ...
            for operation in pkg["uninstall-override-command"]:
                if "admin" in list(operation.keys()):
                    if operation["admin"] == True:
                        if not is_admin():
                            write(
                                "Uninstallation Must Be Run As Administrator",
                                "bright_red",
                                metadata,
                            )
                            os._exit(1)
                ...
                if operation["type"] == "python":
                    ...
                    for line in operation["code"]:
                        code += (
                            line.replace("<display-name>", res["display-name"])
                            .replace("<package-name>", res["package-name"])
                            .replace("<version>", version)
                            + "\n"
                        )
                    exec(code)
        ...
        if not skp:
            if package not in installed_packages:
                ...
                write(
                    f"Could not find any existing installations of {display_name}",
                    "bright_red",
                    metadata,
                )
                try:
        ...
        pkg = pkg[version]
        #
        override_uninstall_switches = (
            pkg["override-default-uninstall-switches"]
            if "override-default-uninstall-switches" in list(pkg.keys())
            else False
        )
        log_info("Generating Packet For Further Installation.", metadata.logfile)
        ...
        if packet.win64_type in ftp:
            if find_msix_installation(pkg["uninstall-bundle-identifier"]):
                if uninstall_msix(pkg["uninstall-bundle-identifier"]) == 0:
                    write(
                        f"Successfully Uninstalled {packet.display_name}",
                        "bright_green",
                        metadata,
                    )
                    close_log(metadata.logfile, "Uninstall")
        ...
        add_hotkey("ctrl+c", lambda: kill_proc(proc, metadata))
        ...
        if not metadata.no_color:
            write(
                f"{Fore.LIGHTCYAN_EX}Uninstalling {packet.display_name}{Fore.RESET}",
                "bright_white",
                metadata,
            )
        else:
            write(f"Uninstalling {packet.display_name}", "bright_white", metadata)
        ...
        if "QuietUninstallString" in key:
            ...
            if packet.uninstall_switches:
                if packet.uninstall_switches != []:
                    write_verbose("Adding additional uninstall switches", metadata)
                    write_debug(
            ...
            run_test = run_cmd(
                command.replace("<version>", packet.version),
                metadata,
                "uninstallation",
                packet,
            )
            ...
            if packet.shim:
                ...
                for shim in packet.shim:
                    ...
                    shim = shim.replace(
                        "<install-directory>", replace_install_dir
                    ).replace("<version>", packet.version)
                    shim_name = (
                        shim.split("\\")[-1]
                        .split(".")[0]
                        .replace("<version>", packet.version)
        ...
        if packet.run_test:
            ...
            if not find_existing_installation(packet.json_name, packet.display_name):
                ...
                write(
                    "Retrying Registry Check In 7.5 seconds", "bright_yellow", metadata
                )
                tm.sleep(7.5)
                if not find_existing_installation(
                    packet.json_name, packet.display_name
                ):
                    write(
                        f"[ {Fore.LIGHTGREEN_EX}OK{Fore.RESET} ]  Registry Check",
                        "bright_white",
                    ...
                    write(
                        f"Failed To Uninstall {packet.display_name}",
                        "bright_magenta",
                        metadata,
                    )
                    log_error(
            ...
            log_info(f"Succesfully Uninstalled {packet.display_name}", metadata.logfile)
            write_debug(
                f'Terminated debugger at {strftime("%H:%M:%S")} on uninstall::completion',
                metadata,
            )
...
def cleanup():
    ...
    with Halo("Cleaning Up ", text_color="green") as h:
        ...
        h.stop()
        with Bar(
            f"{Fore.LIGHTCYAN_EX}Deleting Temporary Files{Fore.RESET}",
            max=len(files) - sub,
            bar_prefix=" [ ",
            bar_suffix=" ] ",
            fill=f"{Fore.LIGHTGREEN_EX}={Fore.RESET}",
            empty_fill=f"{Fore.LIGHTBLACK_EX}-{Fore.RESET}",
        ) as b:
            for f in files:
                if f != "configurations":
                    try:
...
@click.option(
    "--remove", "-uninst", is_flag=True, help="Uninstall packages in a bundle installed"
)
@click.option(
    "--verbose", "-v", is_flag=True, help="Enable verbose mode for bundle installation"
)
@click.option(
    "--debug", "-d", is_flag=True, help="Enable debug mode for bundle installation"
)
@click.option(
    "--no-progress",
    "-np",
    is_flag=True,
    default=False,
    help="Disable progress bar for bundle installation",
)
@click.option(
    "--no-color",
    "-nc",
    is_flag=True,
    help="Disable colored output for bundle installation",
)
@click.option("--log-output", "-l", "logfile", help="Log output to the specified file")
@click.option(
    "--install-dir",
...
@click.option(
    ...
    "-vc",
    is_flag=True,
    help="Check for virus before bundle installation",
)
@click.option(
    "-y", "--yes", is_flag=True, help="Accept all prompts during bundle installation"
)
@click.option(
    "--silent",
    "-s",
    is_flag=True,
    help="Completely silent bundle installation without any output to console",
)
@click.option(
    "--sync",
    "-sc",
...
@click.option(
    ...
    "-rd",
    is_flag=True,
    help="Cleanup all traces of package after bundle installation",
)
@click.option(
    "--exclude", "-ex", "exclude", help="Exclude a package from bundle installation"
)
@click.option("--rate-limit", "-rl", type=int, default=-1)
@click.pass_context
def bundle(
    ...
    if is_admin():
        ...
        click.echo(
            click.style(
                "\nAdministrator Elevation Required For Bundle Installation. Exit Code [0001]",
                "bright_red",
            ),
            err=True,
        )
        disp_error_msg(
            get_error_message("0001", "installation", "None", None, metadata, ""),
            metadata,
        )
...
@click.option(
    ...
    "-sw",
    is_flag=True,
    help="Find packages which start with the specified literal",
)
@click.option(
    "--exact",
    "-e",
    is_flag=True,
    help="Find packages which exactly match the specified literal",
)
def search(
    approx_name: str,
    starts_with: str,
    ...
    if starts_with:
        ...
        matches = get_close_matches(approx_name, correct_names, cutoff=0.7)
    ...
    if len(matches) > 0:
        ...
        if len(matches) != 1:
            click.echo(
                click.style(f"{len(matches)} packages found.", fg="bright_green")
            )
        else:
        ...
        if len(matches) > 0:
            ...
            for match in matches:
                if idx == 0:
                    click.echo(click.style(f"{match}", fg="bright_black"))
                    idx += 1
                    continue
                else:
                    print(match)
...
def new(project_name: str):
    ...
    click.echo(
        click.style(
            f"Successfully Created {Fore.LIGHTBLUE_EX}`{project_name}.electric`{Fore.LIGHTGREEN_EX} at {os.getcwd()}\\",
            "bright_green",
        )
    )
...
@click.option(
    ...
    "-id",
    "install_dir",
    help="Register a specific installation directory",
)
def register(package_name: str, version: str, install_dir: str):
    ...
    with open(
        rf"{PathManager.get_appdata_directory()}\Current\{package_name}@{latest_version}.json",
        "w+",
    ) as f:
        f.write(
            json.dumps(
...
@click.option(
    ...
    "-iv",
    is_flag=True,
    help="Include versions for the config installation",
)
@click.option(
    "--verbose", "-v", is_flag=True, help="Enable verbose mode for config installation"
)
@click.option(
    "--debug", "-d", is_flag=True, help="Enable debug mode for config installation"
)
@click.option(
    "--no-progress",
    "-np",
    is_flag=True,
    default=False,
    help="Disable progress bar for config installation",
)
@click.option(
    "--no-color",
    "-nc",
    is_flag=True,
    help="Disable colored output for config installation",
)
@click.option("--log-output", "-l", "logfile", help="Log output to the specified file")
@click.option(
    "--install-dir",
...
@click.option(
    ...
    "-vc",
    is_flag=True,
    help="Check for virus before config installation",
)
@click.option(
    "-y", "--yes", is_flag=True, help="Accept all prompts during config installation"
)
@click.option(
    "--silent",
    "-s",
    is_flag=True,
    help="Completely silent config installation without any output to console",
)
@click.option(
    "--sync",
    "-sc",
...
@click.option(
    ...
    "-rd",
    is_flag=True,
    help="Cleanup all traces of package after config installation",
)
@click.option("--rate-limit", "-rl", type=int, default=-1)
def config(
    config_path: str,
...
def sign(filepath: str):
    ...
    with open(filepath, "r") as f:
        ...
        if (
            "# --------------------Checksum Start-------------------------- #" in l
            and "# --------------------Checksum End--------------------------- #" in l
        ):
            click.echo(
                click.style("File Already Signed, Aborting Signing!", fg="bright_red")
            )
            sys.exit()
...
def generate(filepath: str):
    ...
    if use_editor:
        ...
        if include_editor:
            editor = prompt(
                "Enter The Development Text Editor You Use => ",
                completer=editor_completion,
    ...
    include_npm = None
    try:
        proc = Popen("npm --version", stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True)
        _, err = proc.communicate()
        if not err:
            include_npm = confirm(
    ...
    if filepath:
        ...
        with open(
            f"{PathManager.get_desktop_directory()}\electric-configuration.electric",
            "w+",
        ) as f:
            f.writelines(
                get_configuration_data(
...
def ls(_, installed: bool, versions: bool):
    ...
    if installed:
        ...
        installed_software = (
            send_query(winreg.HKEY_LOCAL_MACHINE, winreg.KEY_WOW64_32KEY)
            + send_query(winreg.HKEY_LOCAL_MACHINE, winreg.KEY_WOW64_64KEY)
            + send_query(winreg.HKEY_CURRENT_USER, 0)
        )
...
@click.option(
    ...
    "--pre-release",
    is_flag=True,
    help="Show a nightly or pre-release build of a package",
)
@click.argument("package_name", required=True)
...
def autoupdate(
    ...
    for tag in soup.find_all(
        ...
        if tag:
            try:
                version_list[
                    tag.find("a").text.strip().replace("v", "").replace("V", "")
                ] = int(
                    tag.find("a")
                    .text.strip()
                    .replace(".", "")
    ...
    int_web_version = int(
        web_version.strip().replace("v", "").replace("V", "").replace(".", "")
    )

    try:
        int_current_version = int(
            latest_version.strip().replace("v", "").replace("V", "").replace(".", "")
        )
    except:
    ...
    if int_current_version < int_web_version:
        ...
        colorful_json = json.dumps(data, indent=4)
        print(colorful_json)
...
Command: electric feature [enable|disable] electric-progress-bar
        ...
        if feature in [
            ...
            if feature == "support-message" and method == "disable":
                ...
                with open(
                    rf"{PathManager.get_appdata_directory()}\settings.json", "w+"
                ) as f:
